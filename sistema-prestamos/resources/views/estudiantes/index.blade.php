                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-3">
                        <select name="tipo" class="form-select">
                            ...
                            <option value="estudiante" {{ request('tipo') == 'estudiante' ? 'selected' : '' }}>Estudiante
                                Regular</option>
                            <option value="practicante" {{ request('tipo') == 'practicante' ? 'selected' : '' }}>Practicante
                                (Staff)</option>
                        </select>
                    </div>
                    ...
                    <tbody>
                        @forelse ($estudiantes as $estudiante)
                            <tr>
                                ...
                                <td>
                                    <span @if($estudiante->observaciones) data-bs-toggle="tooltip" data-bs-placement="top"
                                    title="{{ $estudiante->observaciones }}" @endif>
                                        {{ $estudiante->nombre }} {{ $estudiante->apellido }}
                                    </span>
                                </td>
                                <td>{{ $estudiante->email }}</td>
                                <td>{{ $estudiante->carrera_id ? optional($estudiante->carreraRelacion)->nombre : ($estudiante->carrera ?: '-') }}</td>
                                ...
                                <td class="text-end">
                                    ...
                                    <form action="{{ route('estudiantes.destroy', $estudiante) }}" method="POST"
                                        ...
                                        <button type="submit" class="btn btn-sm btn-outline-danger"
                                            onclick="return confirm('¿Seguro que deseas eliminar a este estudiante?')"
                                            title="Eliminar">
