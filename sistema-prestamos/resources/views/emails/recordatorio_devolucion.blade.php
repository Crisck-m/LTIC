...
<body>
    ...
    <ul>
        <li><strong>Equipo:</strong> {{ $prestamo->equipo->tipo }} - {{ $prestamo->equipo->marca }} (Serie: {{ $prestamo->equipo->codigo_puce }})</li>
        <li><strong>Fecha de Préstamo:</strong> {{ $prestamo->fecha_prestamo->format('d/m/Y H:i') }}</li>
        <li><strong>Fecha Esperada de Devolución:</strong> {{ $prestamo->fecha_devolucion_esperada ? $prestamo->fecha_devolucion_esperada->format('d/m/Y H:i') : 'No especificada' }}</li>
    </ul>
